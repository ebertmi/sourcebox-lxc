...
var errnoException = util._errnoException || function (errno, syscall) {
  errno = -errno;
...
function exitCallback(attachedProcess, exitCode, signalCode) {
  ...
  if (exitCode < 0) {
    var err = errnoException(exitCode, 'spawn ' + attachedProcess.spawnfile);
    err.path = attachedProcess.spawnfile;
